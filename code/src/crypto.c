...
/*basic logic functions Ch and Maj*/
...
/* the 4 sigma functions
   SIGMA--> used in the compression, they only rotate
   sigma--> used to prepare the words, they end with a shift
*/
...
int hex16_to_u64(const char *in, uint64_t *out){
    ...
    //convert the string and save it in out
...
/* ========================================================================
    padding and message preparation to get 512 bit (64 byte) blocks
 ==========================================================================*/
...
int sha256_hex(const void *data, size_t len, char out[65]){
    ...
    // create padded_msg and allocate the memory, adding the 0s
    uint8_t *padded_msg = (uint8_t *)calloc(padded_len, sizeof(uint8_t));
    if(padded_msg == NULL){
        return SYS_ERROR; //if the pc has no RAM, error
    }

    // put the original data at the start of the new array
    memcpy(padded_msg, data, len);

    // add the padding bit
    padded_msg[len] = 0x80; // 0x80 is 10000000

    // put the length in bits in the last 8 bytes at the end
    for (int i = 0; i < 8; i++) {
        padded_msg[padded_len - 1 - i] = (uint8_t)(msg_len_bits >> (i * 8));
    }

    // go through the message one 64 byte block at a time
    for (size_t offset = 0; offset < padded_len; offset += 64) {

        uint32_t W[64]; // array of 64 words of 32 bits

        for(int i = 0; i < 16; i++){
            // copy the 16 chunks of 32 bits of the current block
        ...
        // generate the remaining 48 words with the sigma functions
        for(int i = 16; i < 64; i++){
            W[i] = sigma1(W[i - 2]) + W[i - 7] + sigma0(W[i - 15]) + W[i - 16];
        }

        // start the state variables from the current intermediate hash
        uint32_t a = H[0];
        uint32_t b = H[1];
        uint32_t c = H[2];
        uint32_t d = H[3];
    ...
    free(padded_msg); // free the memory allocated for the message
     return OK; // return OK
...
int merkle_root(const char *const *txs, size_t n, char out[65]){
    ...
    //check if the pc has RAM, otherwise exit with an error
    ...
    //two pointers to swap the tables at the end of every level
    ...
    //odd case: prepare the empty string
    char empty_hash[65];
    ...
    while(current_count > 1){
     ...
     strcpy(out, read_level[0]); // copy the Merkle root into the output
